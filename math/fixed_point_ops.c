...
int16_t fp_tanh(int16_t x, uint16_t precision) {
    ...
    int16_t one_half = 1 << (precision - 1);
    int16_t one = int_to_fp(1, precision)-1; //koo: I made to positive 0.999
...
int16_t fp_sigmoid(int16_t x, uint16_t precision) {
    ...
    int16_t one = (1 << precision); //-1;  //koo: if precision is 15, one is -32768 so I made it to 32767
    int16_t one_half = 1 << (precision - 1);

    volatile int16_t half_x = fp_mul(x, one_half, precision);
    volatile int16_t tanh = fp_tanh(half_x, precision);
    volatile int16_t temp = fp_mul(tanh, one_half, precision);
    volatile int16_t result = fp_add(temp, one_half);
    //volatile int16_t result = fp_mul(fp_add(tanh, one), one_half, precision);
